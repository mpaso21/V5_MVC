...
/**
 * Metodo FruitoreModel rappresenta il modello nonch� la logica applicativa
...
/**
 * Metodo FruitoreModel � costituito da una lista attuali di fruitori, da un archivio
...
//				if(fr.getId().equals(id)) {//controllo che l'id creato non sia gi� assegnato a un fruitore
//					flag = true;
//					break;
//				}
//			}
...
//		f.setId(id);//ID UNIVOCO PER IDENTIFICARE IL FRUITORE
	...
	public boolean controlloIdGiaEsistente(String id){
		for(Fruitore fr : fruitori){
			if(fr.getId().equals(id)&& id.equalsIgnoreCase(ConstantsCittadino.NOME_OPERATORE)) {//controllo che l'id creato non sia gi� assegnato a un fruitore
				return true;
			}
		}
		return false;
	...
	public void rimozioneFruitore(Fruitore f){
		...
		if(fruitori!=null && !fruitori.isEmpty()){//IS EMPTY RITORNA TRUE SE L'ELENCO E' VUOTO
			fruitori.remove(f);  				//NULL-> SE ELENCO NON � STATO INIZIALIZZATO
		}
	}

	/**
	 *Il metodo controlloScadenza permette di controllare, per ogni fruitore, la data di scadenza 
	 *dell'iscrizione. Se la data � scaduta, il fruitore viene rimosso dall'elenco dei fruitori.
	 * @param d
	 */
	public void controlloScadenza(LocalDateTime d){
		if( !fruitori.isEmpty()){//d data corrente in cui controllo
			...
			Fruitore f;
			for(int i = fruitori.size()-1; i>=0; i--){//se la data scadenza � prima della data corrente -> data scadenza gi� scaduta e quindi  il fruitore viene eliminato dall'elenco
...
// * @return
// */
//	public boolean controlloScadenza(Fruitore f) {//dovr� essere richiamato in controllloScadenza per tutti 
...
//	}
	 ...
	 * 	Questo metodo � utilizzato ogni volta che il fruitore seleziona un'opzione dal men�.
	 * @return
	 */
...
//	 * ovviamente se la richiesta di rinnovo � effettuata prima della scadenza dell'iscrizione e dopo
